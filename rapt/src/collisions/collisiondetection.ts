...
namespace CollisionDetector {
    ...
    // collisions
    export function collideEntityWorld(entity, ref_deltaPosition, ref_velocity, elasticity, world, emergency) {
        return collideShapeWorld(entity.getShape(), ref_deltaPosition, ref_velocity, elasticity, world, entity.getColor(), emergency);
    }
    export function collideShapeWorld(shape, ref_deltaPosition, ref_velocity, elasticity, world, color, emergency) {
    ...
    // overlaps
    export function overlapShapePlayers(shape) {
        var players = [];
    ...
    export function overlapPlayers() {
    ...
    export function onEntityWorld(entity, edgeQuad, world) {
    ...
    export function closestToEntityWorld(entity, radius, ref_shapePoint, ref_worldPoint, world) {
    ...
    export function containsPointShape(point, shape) {
    ...
    export function intersectEntitySegment(entity, segment) {
    ...
    // INTERSECTIONS
    export function intersectSegments(segment0, segment1, ref_segmentProportion0, ref_segmentProportion1, ref_contactPoint) {
    ...
    export function intersectCircleLine(circle, line, ref_lineProportion0, ref_lineProportion1) {
    ...
    export function intersectShapeSegment(shape, segment) {
        switch (shape.getType()) {
            case SHAPE_CIRCLE:
    ...
    export function intersectCircleSegment(circle, segment) {
    ...
    export function intersectPolygonSegment(polygon, segment) {
    ...
    // COLLISIONS
    export function collideShapeSegment(shape, deltaPosition, segment) {
    ...
    export function collideCircleSegment(circle, deltaPosition, segment) {
        var segmentNormal = segment.normal;
    ...
    export function collideCirclePoint(circle, deltaPosition, point) {
    ...
    export function collidePolygonSegment(polygon, deltaPosition, segment) {
        ...
        for (i = 0; i < polygon.vertices.length; i++) {
            ...
            for (var j = 0; j < 2; j++) {
                ...
                if (!intersectSegments(polygonSegment,
    ...
    export function emergencyCollideShapeWorld(shape, ref_deltaPosition, ref_velocity, world) {
    ...
    // OVERLAPS
    export function overlapShapes(shape0, shape1) {
    ...
    export function overlapPolygons(polygon0, polygon1) {
        var i;
    ...
    // CONTAINS
    export function containsPointPolygon(point, polygon) {
        var len = polygon.vertices.length;
        for (var i = 0; i < len; ++i) {
            // Is this point outside this edge?  if so, it's not inside the polygon
            if (point.sub(polygon.vertices[i].add(polygon.center)).dot(polygon.segments[i].normal) > 0) {
    ...
    // DISTANCES
    export function distanceShapeSegment(shape, segment) {
    ...
    export function distanceShapePoint(shape, point) {
        switch (shape.getType()) {
            case SHAPE_CIRCLE:
    ...
    export function distanceCirclePoint(circle, point) {
        var distance = circle.center.sub(point).length();
        return distance > circle.radius ? distance - circle.radius : 0;
    };
    export function distancePolygonPoint(polygon, point) {
    ...
    // CLOSEST TO
    export function closestToShapeSegment(shape, ref_shapePoint, ref_segmentPoint, segment) {
        switch (shape.getType()) {
            case SHAPE_CIRCLE:
    ...
    export function closestToCircleSegment(circle: Circle, ref_shapePoint, ref_segmentPoint, segment) {
    ...
    export function closestToPolygonSegment(polygon, ref_shapePoint, ref_segmentPoint, segment) {
        var distance = Number.POSITIVE_INFINITY;
        var thisDistance;
    ...
    // PENETRATIONS
    export function penetrationEntityWorld(entity, edgeQuad, world) {
        var shape = entity.getShape();
    ...
    export function penetrationShapeSegment(shape, segment) {
        switch (shape.getType()) {
            case SHAPE_CIRCLE:
    ...
    export function penetrationCircleSegment(circle, segment) {
        // a directed radius towards the segment
