...
class Vector {
    ...
    eq(v) { return Math.abs(this.x - v.x) + Math.abs(this.y - v.y) < 0.001; };

    // inplace operations
    inplaceNeg() { this.x = -this.x; this.y = -this.y; };
    inplaceAdd(v) { this.x += v.x; this.y += v.y; };
    inplaceSub(v) { this.x -= v.x; this.y -= v.y; };
    ...
    angleBetween(v) { return this.atan2() - v.atan2(); };
    rotate(theta) { var s = Math.sin(theta), c = Math.cos(theta); return new Vector(this.x * c - this.y * s, this.x * s + this.y * c); };
    minComponents(v) { return new Vector(Math.min(this.x, v.x), Math.min(this.y, v.y)); };
    maxComponents(v) { return new Vector(Math.max(this.x, v.x), Math.max(this.y, v.y)); };
    ...
    static lerp(a, b, percent) { return a.add(b.sub(a).mul(percent)); };
}
